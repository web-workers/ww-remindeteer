...
(async () => {
    ...
    // If necessary, create a message to send to the WW crew.
    let msg = "";

    if (moment(lastCommit).isBefore(moment().subtract(1, "month"))) {
        msg += "The website is old.\r\n";
    }

    if (isDescriptionMissing) {
        msg += "Meetup.com page needs a description.";
    }

    if (msg.length) {
        console.log(msg);
        // Send brob and eterps a message
    }
...
async function checkMeetup(browser) {
    ...
    await meetupPage.waitForSelector("div.eventList");
